...
| [Aggregate Byte and Time](#aggregate-byte-and-time) | Aggregates byte size and time columns with optional output units and aggregation type |
...
## Aggregate Byte and Time

The `aggregate-byte-time` directive allows you to aggregate two types of data across rows:
- **Byte sizes** like `10KB`, `1.5MB`
- **Time durations** like `5ms`, `2.1s`

It supports output in custom units (e.g., MB, seconds) and aggregation types (e.g., total, average).

### Syntax

```text
aggregate-byte-time :<sourceByteColumn> :<sourceTimeColumn> <outputByteColumn> <outputTimeColumn> [byteUnit] [timeUnit] [aggregationType]
